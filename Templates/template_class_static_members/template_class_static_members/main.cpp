...
template <class T> struct A {
  ...
  A() { p = new T; counter++; }
  ~A() { delete p; counter--; }
// declaration template static field
...
template <class T> void A<T>::getCOUNTER() {
  cout << typeid(T).name() << " - objects is " <<
  counter << '\n'; }

template <class T> void A<T>::getCOUNTERT() {
cout << typeid(T).name() << " - objects is " <<
 (int)A<T>::counterT << '\n'; }

template <class T> T A<T>::getCounterT(T a) {
T i = a; cout << typeid(T).name() <<
 " - objects is " << (int)A<T>::counterT << '\n';
return i; }
